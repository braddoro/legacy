...
Do Until objRS Is Nothing
	...
	For x = 0 to (objRS.Fields.count - 1)
		if not ((objRS.Fields(x).Name = "EstimatedTime") or (right(objRS.Fields(x).Name,2) = "ID") or (right(objRS.Fields(x).Name,5) = "ID_fk")) then
	...
	Do While Not objRS.EOF
		response.write "<tr>"
		For x = 0 to (objRS.Fields.count - 1)
			if ((objRS.Fields(x).Name = "EstimatedTime") or objRS.Fields(x).Name = "MeetingDate" or objRS.Fields(x).Name = "SegmentTask" or objRS.Fields(x).Name = "SegmentType") then
				...
				if objRS.Fields(x).Name = "MeetingDate" then
					response.write "<td>" & formatdatetime(dateadd("n",MeetingTime,objRS.Fields("MeetingDate")),3) & " (" & objRS.Fields("EstimatedTime") & ")" & "</td>"
					MeetingTime = MeetingTime + objRS.Fields("EstimatedTime")
				end if
			else
