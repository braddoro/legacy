...
<html>
<head>
<title>Merit Badge Detail</title>
...
<td>
<%
str_SQL = "SELECT BooksOnLoan.BooksOnLoanID, [FirstName]+' '+[LastName] AS Member, MeritBadgeBooks.MeritBadgeBook, BooksOnLoan.MemberID_fk, BooksOnLoan.MeritBadgeID_fk, BooksOnLoan.LastDate, BooksOnLoan.Amount, BooksOnLoan.BookStatusID_fk, BookStatuses.BookStatus FROM MeritBadgeBooks INNER JOIN (BookStatuses INNER JOIN (Members INNER JOIN BooksOnLoan ON Members.MemberID = BooksOnLoan.MemberID_fk) ON BookStatuses.BookStatusID = BooksOnLoan.BookStatusID_fk) ON MeritBadgeBooks.MeritBadgeBookID = BooksOnLoan.MeritBadgeID_fk WHERE (((BooksOnLoan.MeritBadgeID_fk)=" & request("ID") & "));"

set obj_RecordSet = server.createObject("ADODB.Recordset")
obj_RecordSet.Open str_SQL, str_ConnectionString

if not (obj_RecordSet.bof and obj_RecordSet.eof) then
	RESPONSE.write "<h2>Books on Loan for " & obj_RecordSet("MeritBadgeBook") & "</h2><br><br>"
end if
